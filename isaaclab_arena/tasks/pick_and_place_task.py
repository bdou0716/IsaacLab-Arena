...
class PickAndPlaceTask(TaskBase):
    ...
    As a standalone task, success also requires completing the tracked settle and lift stages first.
    ...
    def get_progress_objectives(self) -> list[ProgressObjective]:
        """Return the settle, lift, and place stages used for standalone task success and progress."""
        return [
            ProgressObjective(
                name="pick_and_place",
                predicate_groups=[
